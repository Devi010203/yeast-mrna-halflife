...
# -*- coding: utf-8 -*-
"""
Bland–Altman Analysis (Full Test Set)
Output Directory: result/plot/bland_altman/<timestamp>/
Images: PNG+SVG, dpi=400; Coordinate labels non-italic

Generate:
  1) Bland–Altman (linear scale)
  2) Bland–Altman (log1p scale)
  3) MAE by mean-decile (auxiliary heteroscedasticity assessment)
  4) summary.csv / summary.txt (bias, SD, LoA, correlation, etc.)
"""
...
RUN_DIR   = r"Path\To\Your\Training\Output\Directory"   #  Change it to your complete training output directory.
INPUT_CSV = "final_test_predictions.csv"       #  Change to actual filenames (must include predictions and actual values)
DPI       = 400
POINT_SIZE   = 10
POINT_ALPHA  = 0.25
NBINS_DECILE = 10

QTRIM_X = (0.0, 1.0)   # Crop the display range along the x-axis by quantiles (eliminating the visual impact of outliers)
QTRIM_Y = (0.0, 1.0)   # The y-axis display range is clipped according to quantiles.
Y_ZERO_CENTER = True     # Position the horizontal line y=0 at the center of the image (symmetrical upper and lower boundaries).
LIM_PAD_FRAC = 0.1      # Additional margin ratio at both ends
...
def _limits_from_quantiles(arr: np.ndarray, qlo: float, qhi: float, pad: float = 0.03) -> tuple[float, float]:
    """Display the range based on the quantiles, adding a small margin."""
...
def _loa_stats(diff: np.ndarray, alpha: float = 0.05) -> Dict[str, float]:
    """Mean difference, SD, ±1.96SD; simultaneously providing an approximate 95% CI for the mean and LoA (Bland & Altman 1986 simplified)"""
    n = diff.size
    ...
    # LoA Approximation CI (Common Approximation Formula)
...
def _fit_trend(x: np.ndarray, y: np.ndarray) -> Tuple[float,float,float]:
    """Simple linear regression y = a + bx, returning a, b, and p-value (testing whether the slope is zero)"""
...
def _plot_ba(x_mean: np.ndarray, diff: np.ndarray, title: str, out_base: Path):
    st = _loa_stats(diff)               # Statistics still use full data; if you need to trim even the statistics, switch to diff_mask before calculating.
    a, b, p = _fit_trend(x_mean, diff)

    fig, ax = plt.subplots(figsize=(6.8, 4.8))
    ax.scatter(x_mean, diff, s=POINT_SIZE, alpha=POINT_ALPHA, linewidth=0)

    # === New: First set the display range (percentile clipping) ===
    # x-axis trimmed by quantile
    xlo, xhi = _limits_from_quantiles(x_mean, QTRIM_X[0], QTRIM_X[1], pad=LIM_PAD_FRAC)
    ax.set_xlim(xlo, xhi)

    # y Split by axis; if centered at 0, reflect across |max(|qlo|, |qhi|)|
    ...
    # Mean Difference & Consistency Boundary (At this point, xlim is fixed, and the shading correctly covers the full width)
    ...
    # Approximate CI for the mean difference and LoA (light-colored band)
    xl0, xl1 = ax.get_xlim()
    ...
    # Trend Fitting (Testing for Proportionality Deviation)
...
def main():
    ...
    if not fp.is_file():
        raise FileNotFoundError(f"No input found:{fp}")
    ...
    # log1p scale (provides a more stable view of proportionality deviations)
    ...
    print("[Completed] Output to:", outdir)
