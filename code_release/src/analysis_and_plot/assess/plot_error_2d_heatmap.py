...
# -*- coding: utf-8 -*-
"""
Error 2D Heatmap: Length × GC
Generation:
  - Equal-width binning: MAE / Bias / Count heatmap + CSV
  - Quantile binning: MAE / Bias / Count heatmap + CSV
Output: result/plot/error_2d_heatmap/<timestamp>/ (PNG+SVG, dpi=400)
"""
...
# Box Splitting Settings (Equal Width & Percentile)
...
# Maximum number of ticks displayed on the axis to avoid overlapping
...
def _ensure_len_gc(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "sequence" in out.columns:
        ...
        out["gc_frac"] = G
        return out
    # fallback: Use existing columns directly
    ...
    if len_col is None or gc_col is None:
        raise ValueError("Missing sequence or length/gc columns, 2D heatmap cannot be calculated.")
...
def _digitize(x: np.ndarray, edges: np.ndarray) -> np.ndarray:
    idx = np.digitize(x, edges, right=True) - 1  # Left-closed, right-open; the last cell includes the right end.
...
def _grid_agg(len_vals: np.ndarray, gc_vals: np.ndarray, resid: np.ndarray,
    ...
    # For speed, use group aggregation.
    ...
    # Flatten to a table and include the border.
    rows = []
    for li in range(nL):
        for gi in range(nG):
            rows.append({
...
def _plot_heat(mat: np.ndarray, len_edges: np.ndarray, gc_edges: np.ndarray,
    ...
    # x/y scale (using bin indexing + partial center value labels to avoid overlapping long interval strings)
